#!/usr/bin/env python3
from . import client
from . import server
import argparse
import sys


def main():
    parser = argparse.ArgumentParser(description="A dictation software package for Linux. Running this command without arguments either starts a new recording or ends one, copying the result to the clipboard.")
    parser.add_argument("--paste", action="store_true", help="run as normal but also automatically paste the transcription result in the currently focussed application")
    parser.add_argument("--cancel", action="store_true", help="cancel the ongoing recording")
    parser.add_argument("--stop-server", action="store_true", help="stop the background transcription server now instead of when it has been idle for five minutes, freeing the memory held by the model; the next dictation starts it again")
    parser.add_argument("--finish-setup", action="store_true", help=argparse.SUPPRESS)  # hidden argument
    args = parser.parse_args()

    if args.stop_server:
        if args.paste or args.cancel or args.finish_setup:
            parser.error("--stop-server cannot be combined with other options")
        return stop_server()

    if args.finish_setup:
        server.load_model()  # load the model to finish setup and then exit
        return

    if args.cancel:
        # Cancelling discards the recording, so it has no use for a server.
        client.cancel_recording()
    else:
        # try to start the server; this will fail silently if it's already up
        server.start()
        client.main(paste=args.paste)


def stop_server():
    """Stop the transcription server, reporting the outcome; return the exit status."""
    try:
        was_running = server.stop()
    except (RuntimeError, OSError) as exc:
        print(f"whspr: could not stop the server: {exc}", file=sys.stderr)
        return 1
    if was_running:
        print("Stopped the whspr server.")
    else:
        print("The whspr server is not running.")
    return 0


if __name__ == "__main__":
    sys.exit(main())
